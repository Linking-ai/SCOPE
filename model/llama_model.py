...
from model.kv_utils import init_pyramidkv,init_pyramidinfer,init_snapkv,init_H2O,init_StreamingLLM,init_ALLKV
import math
...
def _flash_attention_forward( # Add to fix 4.44.2
...
def llama_flash_attn2_forward_PyramidKV(
    ...
    **kwargs,
    ...
    attn_output = _flash_attention_forward( # modify to fix 4.44.2
...
def llama_attn_forward_PyramidInfer(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    ...
    **kwargs,
    ...
    init_pyramidinfer(self, num_hidden_layers=self.config.num_hidden_layers)
...
def llama_sdpa_attn_forward_PyramidInfer(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
...
def llama_flash_attn2_forward_PyramidInfer(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.LongTensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
...
def llama_attn_forward_H2O(
    ...
    **kwargs,
    ...
    init_H2O(self, num_hidden_layers=self.config.num_hidden_layers)
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        if key_states.shape[-2] == kv_seq_len: # [SnapKV] add kv_cluster
            ...
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
            ##### Use kv_cluster to update kv in decoding phase #####
            # print(past_key_value.type) # <bound method Module.type of DynamicCache()>
            history_key_states, history_value_states = past_key_value[self.layer_idx]
            # print(history_key_states.shape)
            # print(history_value_states.shape)
            key_states_compress, value_states_compress = self.kv_cluster.update_kv_in_decoding(history_key_states, query_states, history_value_states, attention_mask, self.num_key_value_groups)
            # key_states_compress, value_states_compress = self.kv_cluster.update_kv_in_decoding_h2o(history_key_states, query_states, history_value_states, attention_mask, self.num_key_value_groups)
            # replace
            past_key_value.key_cache[self.layer_idx] = key_states_compress
            past_key_value.value_cache[self.layer_idx] = value_states_compress
...
def llama_flash_attn2_forward_ALLKV(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.LongTensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
...
def llama_attn_forward_SnapKV(
    ...
    **kwargs,
    ...
    init_snapkv(self, num_hidden_layers=self.config.num_hidden_layers)
